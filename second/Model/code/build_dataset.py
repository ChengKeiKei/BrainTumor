...
ALL_EXPERIMENTS = ("ExpA_TxNoMol", "ExpA_Tx", "ExpB_TxRadiomic",
                   "ExpC_TxVLM", "ExpD_TxRadVLM",
                   "Exp1", "Exp2", "Exp3", "Exp4")
ALL_RETRIEVERS  = ("none", "beep", "nomic")
...
def build_one_config(experiment: str, retriever: str, reranker: str,
    ...
    for split in splits:
        ...
        for _, row in df.iterrows():
            ...
            # Use the patient-distinctive keyword query for retrieval (BEEP
            # §3.2 fidelity), but keep the human-readable narrative for the
            # LLM prompt. Earlier diagnostics showed that using row["text"]
            # as the BM25 query collapsed retrieval to 14 unique 3-doc sets
            # across 101 patients on Exp1 because the structured-form labels
            # ("Demographics:", "Molecular markers:", "unknown;", ...) are
            # constant across patients and dominated term frequencies.
            query_text = row.get("retrieval_query") or row["text"]
            docs = retrieve_and_rerank(query_text, cfg)
