...
def _resolve_model_id() -> str:
    """Read RAG_MODEL_ID at call time so run_grid.py's env-var override
    (set AFTER `import train as train_mod`) takes effect.

    NEVER capture this as a module-level constant — that pins the value
    at the moment train.py is first imported, which is too early."""
...
def write_timing(result_dir: Path, tag: str, *, elapsed: float,
                 steps: int, batch_size: int, lr: float) -> Path:
...
def train(tag: str | None = None, prompts_dir: str | None = None,
          ...
          logging_steps: int = 10, max_seq_length: int = 3072) -> Path:
    if prompts_dir:
